...
1. `fetch`
2. `setTimeout`
   The browser handles the timer for setTimeout. Once the timer expires, the callback function is pushed into the respective Queue.
3. `setInterval`
4. Promise
   The browser waits for the promise to be resolved. Once the promise is resolved, the `then` callback function is pushed into the respective Queue.
...
## Macro-Task and Macro-Task Queue
