  ...
  (
    ...
    const handleDrop = useCallback(
      (e: React.DragEvent) => {
        ...
        try {
          ...
          if (onError) {
            onError([
              { name: 'drop', error: 'Failed to process dropped files' },
            ]);
          }
        }
    ...
    const removeFile = useCallback(
      (fileId: string) => {
        ...
        // Reset the file input and update its files property
        if (inputRef.current) {
          const dataTransfer = new DataTransfer();
          // Add all files except the removed one to the input
      ...
      [files, required, onError, inputRef]
    );
