...
const meta: Meta<typeof ConcurrentRenderingExample> = {
  title: 'Suspense Examples/05 - Concurrent Rendering',
...
This example demonstrates using useTransition to create smooth, non-blocking UI transitions with Suspense.

### Use Case
- Tab navigation with async content
- Search with live results
- Pagination
- Filtering large lists
- Any UI transition that involves data loading

### Key Features
- Non-blocking state updates
- UI stays responsive during transitions
- Previous content remains visible while loading
- \`isPending\` state for transition indicators
...
const handleChange = (newValue) => {
  startTransition(() => {
    setValue(newValue);
    setResource(fetchResource(newValue));
  });
};
\`\`\`

### Without vs With useTransition

**WITHOUT useTransition:**
- Click tab → Immediate blank state
- Loading spinner replaces content
- UI feels janky
- Can't interact during loading

**WITH useTransition:**
- Click tab → Previous content stays
- Small loading indicator shown
- UI feels smooth
- Can switch tabs mid-loading

### Benefits
- Better perceived performance
- Reduced layout shift
- More responsive feel
- Users can change their mind mid-transition
...
export default meta;
type Story = StoryObj<typeof ConcurrentRenderingExample>;

const sourceCode = `import React, { Suspense, useState, useTransition } from 'react';
